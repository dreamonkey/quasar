    ...
    <div class="advance-scaffolding row justify-center items-end">
      ...
      <div class="col-9 relative-position">
        <div class="column items-end absolute-bottom-right lp-mb--large">
          ...
          <q-btn
            class="btn--call-to-action lp-mt--medium"
            color="lp-accent"
...
<!--          <q-img :src="`sponsor-logos/${src}`" width="200px" v-for="(src, index) in sponsorLogos.gold" :key="index" />-->
    ...
    <div class="text-center social-channels-call-to-action lp-mb--large">
      <q-img src="~assets/landing-page/homepage-background-images/planet.png" :height="$q.screen.gt.xs? '1080px':'1080px'">
        <div class="bg-transparent absolute-bottom">
          ...
          <div class="row justify-center q-mb-xl q-mt-md q-gutter-md">
            <template v-for="(socialLink, linkIndex) in footerSocialLinks" :key="linkIndex">
              <q-btn :label="socialLink.name" class="btn--call-to-action-no-shadow" color="lp-accent" outline type="a" :href="socialLink.href" target="__blank"/>
            </template>
          </div>
        </div>
      </q-img>
...
export default defineComponent({
  ...
  setup () {
    const footerSocialLinks = socialLinks.reverse().slice(0, 4)

    function scrollSectionIntoView (idOfTarget) {
      document.getElementById(idOfTarget).scrollSectionIntoView({ behavior: 'smooth' })
    }
